...
for measure = 1:3
    pValues = results.spin(measure,:);
    h = fdr_bh(pValues,0.05) % if h is 1, then significant.
    % -> gradient and skewness are FWE corrected sig
end
...
[rho,pval] = corr(d_vals(:, measure), gm_bb.gradients{1}(:,2), 'Type', 'Spearman', 'rows', 'complete');
